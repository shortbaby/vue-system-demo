<template>
  <div class="btn-group" v-bind:class="{open: show}">
    ...
    >
      ...
      <span class="btn-content">{{{ selectedItems.label }}}</span>
      <span class="caret"></span>
    </button>
    <ul class="dropdown-menu select-menu" :style="{minWidth: minWidth + 'px'}">
      <template v-if="options.length">
        <li v-if="search" class="bs-searchbox">
          <input type="text" placeholder="Search" v-model="searchText" class="form-control" autocomplete="off">
        </li>
        <li v-for="option in options | filterBy searchText " v-bind:id="option.value" style="position:relative" track-by="$index">
