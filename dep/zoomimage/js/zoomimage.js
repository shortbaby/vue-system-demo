	...
	EYE.extend({
		...
		zoomimage: {
			...
			zoomIn: function(el) {
				...
				box.find('>div').hide();
				box.find('img').attr('src', el.zoomimageCfg.src).css({top: 0,left: 0,width: '100%',height: '100%',display: 'block',borderWidth: '0px'});
				box.show();
				box.animate(
					{	width: imgWidth + 'px', 
						height: imgHeight + 'px', 
						top: top + borderAndShadow + 'px', 
						left: left + borderAndShadow + 'px'
					}, el.zoomimageCfg.duration, el.zoomimageCfg.easing, function(){
					box.css({
							top: top + 'px',
							left: left + 'px',
							width: width + 'px',
					...
					box.find('>div:first').find('div.zoomimage_sc').css('height', height - el.zoomimageCfg.shadow*2 + 'px').show();
			...
			init: function(opt) {
				...
				return this.each(function(){
					...
					if (el.href && el.href.toLowerCase().match(EYE.zoomimage.types) != null) {
						...
						if (!titleAttr || titleAttr == '' || titleAttr == false) {
