...
import AllocateStudent from './components/AllocateStudent.vue';
import { getStudentList, batchDel , getSchoolList, exportStudent, getOwnerList, getFuzzyClasses} from './request';
import getSelectOptions from '../../common/function/getSelectOptions';
let options = getSelectOptions();
export default {
	created () {
		this.getStudentListHandler();
		this.$dispatch('showcommonstudent');
		this.getOptions();
	},
	data () {
		return {
	...
	events: {
		...
		roothidedetail() {
			//debugger;
			this.$parent.$emit('showcommonstudent');
		},
		hidedetail() {
		...
		showdetail(row) {
			...
			this.$parent.$emit('showdetail', row.id, 'commonStudent');
		}
	},
	methods: {
		getOptions() {
			let self = this;
			$.when(getSchoolList(), getOwnerList({ type: 1 }))
		...
		batchStudent() {
			...
			if (self.selectStudents.length === 0) {
				dispatch('TOAST', '请先选择需要分配的学员');
				return;
			}
			self.selectStudentIds = [];
			self.selectStudents.forEach(function(item) {
				self.selectStudentIds.push(item.id);
			})
	...
	components: {
		...
		Import: Import,
		QuitClass: QuitClass,
		AllocateStudent: AllocateStudent,
		Notice: Notice,
