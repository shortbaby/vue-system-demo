...
<script>
	...
	export default {
		...
		methods: {
			setStatus(){
				var routeArray = window.location.hash.slice(3).split('/');
				this.current = routeArray[0] || 'home';
				this.currentSubTab = '/' + window.location.hash.slice(3);
			},
			getSubMenu (item) {
				this.current = item.alias;
			...
			forward (item) {
				...
				if (url) {
					...
					this.$root.$broadcast('roothidedetail');
